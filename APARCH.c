...
void aparch_filter(int *status, double *sigma2, double* eps, double *loglik, double *param, double *y, int *T){
  ...
  // loop
  *loglik = 0;
  //logden    = -0.5 *log(2*M_PI) -0.5*(1/delta)*log(sigma2[t]) -0.5*((y[0]-mu)*(y[0]-mu))/pow(sigma2[0], 1/delta);
  //*loglik   += logden;
  for( t=1 ; t<*T ; ++t ){
    sigma2[t] = omega + alpha * pow( (fabs(y[t-1] - mu) + gamma * (y[t-1] - mu)) , delta) + beta * sigma2[t-1];
    eps[t]    = (y[t]-mu)/ pow( sigma2[t], 1/delta );
    
    logden    = -0.5 *log(2*M_PI) -0.5*(1/delta)*log(sigma2[t]) -0.5*((y[t]-mu)*(y[t]-mu))/pow(sigma2[t], 1/delta);
    *loglik   += logden;
  }
}
